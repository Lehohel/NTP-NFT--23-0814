...
__global__ void oneStepRK4(cuDoubleComplex *d_field, cuDoubleComplex *d_res, int numberOfTemporalPoints, float stepSize)
{
...
__host__ void executeKernel(cuDoubleComplex *d_field0, cuDoubleComplex *d_fieldFinal, int threadsPerBlock,
                            int notp, int nopp, float thickness, float centralWavelength, float timeStep)
{
  cuDoubleComplex *A = d_field0;
  cuDoubleComplex *B = d_fieldFinal;
  cuDoubleComplex *C;
  // Launch the search CUDA Kernel
  int blocksPerGrid = (notp + threadsPerBlock - 1) / threadsPerBlock;
  float stepSize = thickness / (nopp-1);
...
{  try
  {
    ...
    cuDoubleComplex *field0 = (cuDoubleComplex *) malloc(sizeof(cuDoubleComplex)*numberOfTemporalPoints);
    ...
    // cuDoubleComplex field0[numberOfTemporalPoints];
    // cuDoubleComplex fieldFinal[numberOfTemporalPoints];
    ...
    cuDoubleComplex *d_field0 = allocateDeviceMemory(numberOfTemporalPoints);
    cuDoubleComplex *d_fieldFinal = allocateDeviceMemory(numberOfTemporalPoints);
